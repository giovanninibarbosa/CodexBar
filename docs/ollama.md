...
The pasted text was read as a cookie header but contains no `wos-session` (or legacy session) cookie. Copy the full
`Cookie:` header from a request to `https://ollama.com/settings` while signed in; a single analytics or theme cookie is
not enough.
